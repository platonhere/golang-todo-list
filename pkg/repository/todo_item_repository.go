...
func (r *ToDoItemPostgres) Update(userId, itemId int, input todo.UpdateItemInput) error {
	...
	// //Выглядить запрос будет след. образом
	// UPDATE todo_items ti
	// SET title=$1, description=$2,title=$3, description=$4
	// FROM lists_items li, users_lists ul
	// WHERE ti.id = li.item_id AND li.list_id = ul.list_id AND ul.user_id=$5 AND ti.id=$6

	// UPDATE todo_items ti
	// SET title='Новая задача', description='Очень важная задача'
	// FROM lists_items li, users_lists ul
	// WHERE ti.id = li.item_id AND li.list_id = ul.list_id
	// AND ul.user_id=5 AND ti.id=2;
	// при args := []interface{}{"Новая задача", "Очень важная задача", 5, 2}
